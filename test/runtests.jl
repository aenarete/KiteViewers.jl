...
@testset "KiteViewers.jl" begin
    ...
    @test isapprox(rad2deg(elevation), 70.31815477683752, rtol=1e-4)
    azimuth = azimuth_east(pos_kite)
    @test isapprox(rad2deg(azimuth), 9.217613192065718, rtol=1e-4)
    force = winch_force(kps4) 
    @test isapprox(force, 495.7191503651492, rtol=1e-4)
end
